...
fun AsynchronousSocketChannel.writeAsync(buffer: ByteBuffer) : CompletableFuture<Int> {
    val promise = CompletableFuture<Int>()

    val writeCallback = object : CompletionHandler<Int, Any?> {
        override fun completed(result: Int?, attachment: Any?) {
    ...
    write(buffer, null, writeCallback)
    return promise
}

fun AsynchronousSocketChannel.readAsync(buffer: ByteBuffer) : CompletableFuture<Int> {
    val promise = CompletableFuture<Int>()

    val readCallback = object : CompletionHandler<Int, Any?> {
        override fun completed(result: Int?, attachment: Any?) {
            promise.complete(result)
        }
    ...
    read(buffer, null, readCallback)
    return promise
}

fun AsynchronousServerSocketChannel.acceptAsync() :
        CompletableFuture<AsynchronousSocketChannel> {
    val promise = CompletableFuture<AsynchronousSocketChannel>()
    val acceptCallback = object : CompletionHandler<AsynchronousSocketChannel, Any?> {
